...
const int sensor[] = {13, 12, 14, 27, 26, 25, 33, 32};
...
void setup() 
{
    ...
    if (!BTSerial.begin("ESP32_Bochtje007")) {  // Initialiseer Bluetooth
        SerialPort.println("Bluetooth initialization failed!");
        while (1); // Stop als Bluetooth niet werkt
    }
    SerialPort.println("Bluetooth is ready to pair");
...
void IRAM_ATTR handleInterrupt() {
    ...
    SerialPort.print("Program status changed: ");
    SerialPort.println(programEnabled ? "Enabled" : "Disabled");
}
...
void onDebug() {
    ...
    SerialPort.print("Diff: ");  // Voeg diff toe
